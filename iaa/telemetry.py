...
def is_pending() -> bool:
    ...
    shared = _load_shared()
    telemetry_cfg = shared.telemetry
    return (
...
def setup() -> None:
    ...
    from iaa.telemetry_screenshot import screenshot_before_send  # noqa: PLC0415
...
def _attach_global_tags() -> None:
    ...
    tag 附加在进程级 global scope 上，确保任意线程产生的
...
def collect_report_context() -> dict[str, str]:
    ...
    全局 tag 附加，这里只收集运行期可能变化的数据：进程自身内存占用、当前配置的设备平台
    与截图方式、模拟器分辨率。所有字段采集失败均静默跳过。
    """
    ctx: dict[str, str] = {}
    ...
    try:
        ...
        config = get_conf()
        ctx['device_platform'] = config.device.lifecycle.type
        ctx['control_impl'] = config.device.control_impl
    except Exception:
